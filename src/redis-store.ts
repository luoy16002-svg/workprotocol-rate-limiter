...
export class RedisRateLimitStore implements RateLimitStore {
  ...
  async consumeFixedWindow(input: StoreConsumeInput): Promise<RateLimitResult> {
  ...
  async consumeSlidingWindow(input: StoreConsumeInput): Promise<RateLimitResult> {
    const member = `${input.now}:${Math.random().toString(36).slice(2)}`;
    const [allowedRaw, count, oldest] = asNumberArray(
      await this.redis.eval(SLIDING_WINDOW_SCRIPT, 1, input.key, input.now, input.windowMs, input.limit, member)
    );
  ...
  async consumeTokenBucket(input: TokenBucketConsumeInput): Promise<RateLimitResult> {
    const [allowedRaw, tokens] = asNumberArray(
...
export function createRedisStore(redis: RedisLike): RedisRateLimitStore {
